#!/usr/bin/env bash
# Publica docs/ (generado por bc_static_generator.py) a GitHub Pages: commit +
# push a main si hay cambios. Requiere GH_PAT (token fine-grained,
# contents:write sobre este repo) en el .env — configurable desde el panel ⚙.
#
...
# Si hay historial local sin subir (desarrollo normal en el repo origen que
# aún no se ha pusheado a mano), se aborta en vez de publicarlo de golpe.
set -euo pipefail
cd "$(dirname "$0")"
...
REMOTE_URL="https://x-access-token:${GH_PAT}@github.com/volteret4/bancamp_imap.git"
